...
use EasySwoole\EasySwoole\Task\AbstractInterface\TaskInterface;
        ...
        try{
            ...
            if(is_string($task) && class_exists($task)){
                ...
                if($ref->implementsInterface(TaskInterface::class)){
                    ...
                    $method = $package->getMethod() ?: 'run';
                    if ($ref->hasMethod($method)) {
                        try{
                            $parentClass = $ref->getParentClass();
                            if ($parentClass !== false && $parentClass->getShortName() == 'CronTab') {
                                $reply = $task->run($taskId, $this->workerIndex);
                            } else {
                                $reply = $task->$method($package->getData(),$taskId,$this->workerIndex);
                            }
